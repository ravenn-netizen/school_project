...
def createTableStudent():
    cmd= "USE SCHOOL"
    cursor.execute(cmd)
    cmd="CREATE TABLE STUDENT(STUDENT_ID VARCHAR(6) PRIMARY KEY, NAME VARCHAR(40), DOB DATE, GENDER CHAR(1), AGE INT, GRADE INT, SECTION CHAR(1), STREAM CHAR(3), TRANSPORT VARCHAR(10), BUS_NO INT, BUS_STOP VARCHAR(40), GUARDIAN VARCHAR(60), TEL CHAR(8), EMAIL VARCHAR(30), ADDRESS VARCHAR(60) , CPR VARCHAR(9), PASSWORD VARCHAR(30))"
    cursor.execute(cmd)
...
def createTableAcademic():
    cmd= "USE SCHOOL"
    cursor.execute(cmd)
    cmd = "CREATE TABLE ACADEMIC(SNO INT PRIMARY KEY, STUDENT_ID VARCHAR(6), TERM VARCHAR(6), SUB1 FLOAT, SUB2 FLOAT, SUB3 FLOAT, SUB4 FLOAT, SUB5 FLOAT, AVG FLOAT, REMARKS varchar(10) )"
    cursor.execute(cmd)
