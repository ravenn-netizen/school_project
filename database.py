...
def createTableStudent():
    cmd="CREATE TABLE STUDENT(STUDENTID INT PRIMARY KEY, NAME VARCHAR(30), DOB DATE, GENDER CHAR(1), CLASS INT, SECTION CHAR(1), STREAM VARCHAR(10), TRANSPORT VARCHAR(10), BUS_NO INT, BUS_STOP VARCHAR(20), GUARDIAN VARCHAR(40), TEL INT, EMAIL VARCHAR(20), PO_BOX INT, ADDRESS VARCHAR(60) , CPR INT)"
    cursor.execute(cmd)
...
def createTableStaff():
    cmd = "CREATE TABLE STAFF(STAFFID INT PRIMARY KEY, NAME VARCHAR(30), DEPARTMENT VARCHAR(20), CPR INT DISTINCT, PASSWORD VARCHAR(10))"
    cursor.execute(cmd)
